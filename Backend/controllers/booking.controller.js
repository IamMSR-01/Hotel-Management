...
export const checkAvailabilityAPI = async (req, res) => {
    ...
    try {
        ...
        console.error("Error checking room availability:", error);
        res.status(500).json({ success: false, error: "Error checking room availability" });
    }
}
...
export const getUserBookings = async (req, res) => {
    try {
        ...
        const bookings = await Booking.find({ user: userId }).populate("room").populate("hotel").sort({ createdAt: -1 });
        res.status(200).json({ success: true, bookings });
    } catch (error) {
        console.error("Error fetching user bookings:", error);
        res.status(500).json({ success: false, error: "Error fetching user bookings" });
    }
}
...
export const getHotelBookings = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: "Error fetching hotel bookings" });
    }
}